...
the record is not counted — see [model-card.md](model-card.md)). The published
cutoffs were then applied to each window.

Reproduce the result with `go test ./internal/predict -run Climatology`, or
rebuild the reference table it uses from the archive API (free, no key):
...
The table is `internal/predict/climatologydata/kenya-5county-2015-2024.json`,
SHA-256 `acc41f6890e10eccd16e0052f533a3e7737dcd0da71d723d968c815541c41c8d`. The
full method, provenance, operating points and limitations are in
...
- The dashboard's Model view leads with it, under "How we checked our own
  proposal".
- [model-card.md](model-card.md) records it as the only evaluation this
...
only whether a cutoff is reachable in the climate record. Whether the reachable
ones predict outbreaks is unknown and untested, and this project makes no claim
that they do.

The reference record's own limits — ERA5 at a single county centroid, ten years
of tail behaviour — are listed in [model-card.md](model-card.md).
